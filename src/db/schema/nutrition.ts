...
export const foods = pgTable(
  ...
  {
    id: uuid("id").defaultRandom().primaryKey(),
    categoryId: uuid("category_id").references(() => foodCategories.id, { onDelete: "set null" }),
    name: text("name").notNull(),
    slug: text("slug").notNull().unique(),
  ...
  (table) => [index("foods_category_active_idx").on(table.categoryId, table.isActive)],
);
...
export const foodAllergens = pgTable(
  ...
  (table) => [uniqueIndex("food_allergens_unique").on(table.foodId, table.allergenId)],
);
...
export const recipes = pgTable(
  ...
  (table) => [index("recipes_discovery_idx").on(table.status, table.isPublic, table.mealType)],
);
...
export const recipeIngredients = pgTable(
  ...
  (table) => [uniqueIndex("recipe_ingredients_order_unique").on(table.recipeId, table.sortOrder)],
);
...
export const recipeSteps = pgTable(
  ...
  (table) => [uniqueIndex("recipe_steps_order_unique").on(table.recipeId, table.sortOrder)],
);
...
export const recipeMedia = pgTable(
  ...
  (table) => [uniqueIndex("recipe_media_unique").on(table.recipeId, table.mediaAssetId, table.role)],
);
...
export const recipeTagLinks = pgTable(
  ...
  {
    recipeId: uuid("recipe_id").notNull().references(() => recipes.id, { onDelete: "cascade" }),
    recipeTagId: uuid("recipe_tag_id").notNull().references(() => recipeTags.id, { onDelete: "cascade" }),
  },
  (table) => [uniqueIndex("recipe_tag_links_unique").on(table.recipeId, table.recipeTagId)],
);
...
export const recipeDietaryPreferences = pgTable(
  ...
  (table) => [uniqueIndex("recipe_dietary_preferences_unique").on(table.recipeId, table.dietaryPreferenceId)],
);
...
export const mealPlans = pgTable(
  ...
  (table) => [index("meal_plans_user_dates_idx").on(table.userId, table.startsOn, table.endsOn)],
);
...
export const mealPlanEntries = pgTable(
  ...
  (table) => [
    ...
    check("meal_plan_entry_content_check", sql`${table.recipeId} is not null or ${table.customName} is not null`),
  ],
);
...
export const mealLogs = pgTable(
  ...
  (table) => [index("meal_logs_user_time_idx").on(table.userId, table.loggedAt)],
);
