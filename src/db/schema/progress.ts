...
export const bodyMeasurements = pgTable(
  ...
  (table) => [
    index("body_measurements_user_time_idx").on(table.userId, table.measuredAt),
  ],
);
...
export const progressPhotos = pgTable(
  ...
  (table) => [
    index("progress_photos_user_date_idx").on(table.userId, table.capturedOn),
  ],
);
...
export const dailyHabitLogs = pgTable(
  "daily_habit_logs",
  {
    ...
    creatineG: numeric("creatine_g", { precision: 4, scale: 1 })
      .default("0")
      .notNull(),
  ...
  (table) => [
    uniqueIndex("daily_habit_logs_user_date_unique").on(
      table.userId,
      table.logDate,
    ),
...
export const personalRecords = pgTable(
  ...
  (table) => [
    index("personal_records_user_type_idx").on(table.userId, table.recordType),
  ],
);
