...
export const exercises = pgTable(
  ...
  (table) => [
    index("exercises_category_active_idx").on(table.categoryId, table.isActive),
  ],
);
...
export const exerciseMuscleGroups = pgTable(
  ...
  (table) => [
    uniqueIndex("exercise_muscle_groups_unique").on(
      table.exerciseId,
      table.muscleGroupId,
      table.role,
    ),
...
export const exerciseEquipment = pgTable(
  ...
  (table) => [
    uniqueIndex("exercise_equipment_unique").on(
      table.exerciseId,
      table.equipmentId,
    ),
  ],
...
export const exerciseMedia = pgTable(
  ...
  (table) => [
    uniqueIndex("exercise_media_unique").on(
      table.exerciseId,
      table.mediaAssetId,
      table.role,
    ),
...
export const workoutPlans = pgTable(
  ...
  (table) => [
    index("workout_plans_user_status_idx").on(table.userId, table.status),
  ],
);
...
export const workoutDays = pgTable(
  ...
  (table) => [
    uniqueIndex("workout_days_plan_day_unique").on(
      table.workoutPlanId,
      table.dayNumber,
    ),
  ],
...
export const workoutDayExercises = pgTable(
  ...
  (table) => [
    uniqueIndex("workout_day_exercises_order_unique").on(
      table.workoutDayId,
      table.sortOrder,
    ),
  ],
...
export const workoutSessions = pgTable(
  "workout_sessions",
  {
    ...
    workoutDayId: uuid("workout_day_id").references(() => workoutDays.id, {
      onDelete: "set null",
    }),
...
export const workoutSessionExercises = pgTable(
  ...
  (table) => [
    uniqueIndex("workout_session_exercises_order_unique").on(
...
export const exerciseSets = pgTable(
  ...
  (table) => [
    uniqueIndex("exercise_sets_number_unique").on(
